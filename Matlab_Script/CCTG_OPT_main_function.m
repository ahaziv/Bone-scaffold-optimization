...
fileID = fopen('Parameters.txt', 'r');
InitData = fscanf(fileID, '%f');
FW = InitData(1)*10^-6;              % fillament width
FD = InitData(2)*10^-6;              % fillament distance
ST = InitData(3)*10^-6;               % slice thickness

ShrinkFactor = 10^-3;               % all vetrtices are multiplied by this factor to scale it down
LossFactor = 10^6;                  % the loss is multiplied by this factor for convenience
aPar = 8.534*10^-8;                       
bPar = 50*10^-6;                
...
VUnitCell = 2*FD*FD*ST;     % the total volume of the unit cell
...
CurveLoss = TrueTotTime*(VUnitCell/TotVgrowth)*LossFactor;

%% calculating the pore size loss function and total loss function
PoreRad = 0.5*((ST^2+FD^2)^(1/2)-FW);
PoreSizeLoss = -aPar*log(PoreRad - bPar)*LossFactor;
